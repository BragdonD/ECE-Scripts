...
(function () {
    ...
    const computeCoursePartAverage = (cp) => {
        for (const grade of cp.grades) {
            cp.average += grade.value * grade.weight;
        }
        return cp;
    }
    ...
    const computeCourseAverage = (course) => {
        ...
        for (const coursePart of course.courseParts) {
            ...
            course.average += coursePart.average * coursePart.weight;
            if (coursePart.weight > 0) {
                availableCP += 1;
            }
        }
        console.log(course.name, course.average, availableCP)
        course.average = course.average / availableCP;
    ...
    const computeModuleAverage = (module) => {
        ...
        for (const course of module.courses) {
            ...
            if (course.coefficient > 0) {
                availableCourses += 1;
            }
        }
        module.average = module.average / availableCourses;
        return module;
    }
